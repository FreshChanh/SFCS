...
export default class Food extends Component {
  ...
  componentDidMount() {
    // Fetch food data
    fetch(config.getPath("api/menu/food"), {
      method: 'GET',
      headers: {
    ...
    .then((json) => {
      if (json.status == 1) {
        public_config.createAlert("Can't fetch best foods data from server!");
      }
  ...
  render() {
    ...
    if (this.state.showSearchBar) {
      ...
      header = <Header
        ...
        centerComponent={{ text: 'FOOD', style: { color: '#fff', fontSize: 18} }}
        rightComponent={
